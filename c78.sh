...
URL="https://github.com/caonimagfw/ssr/raw/master/ss322.tar.gz"
BASE=`pwd`
...
function getData()
{
    ...
    read -p "(Default: ${dport}):" port
    [ -z "${port] && portdport}
    expr ${port 1 &>/dev/null
    if [ $? -eq 0 ]; then
        if [ ${portge 1 ] && [ ${portle 65535 ] && [ ${port1} != 0 ]; then
            echo "-------------------------"
            echo "Port = ${port}
            echo "-------------------------"
            break
     ...
     cat > /etc/shadowsocksR.json<<-EOF
{
    ...
    "obfs":"${obfs}",
    "obfs_param":"",
    "redirect":["*:${port}#127.0.0.1:${caddyportinput}"],
    "dns_ipv6":true
    "fast_open":false,
    "workers":50
}
...
Documentation=https://caonima.com/
