...
Download(){
	...
	cd "${FOLDER}"
	if [[ ${bit} == "x86_64" ]]; then
		#https://github.com/caonimagfw/ssr/raw/master/go2/shadowsocks2-linux.gz
		#wget --no-check-certificate -N "https://github.com/shadowsocks/go-shadowsocks2/releases/download/${new_ver}/shadowsocks2-linux.gz"
		wget --no-check-certificate -N "https://github.com/caonimagfw/ssr/raw/master/go2/shadowsocks2-linux.gz"
	else
...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://github.com/caonimagfw/ssr/raw/master/go2/ss_go_centos.sh" -O /etc/init.d/ss-go; then
		...
		if ! wget --no-check-certificate "https://github.com/caonimagfw/ssr/raw/master/go2/ss_go_debian.sh" -O /etc/init.d/ss-go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
