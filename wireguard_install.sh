...
#判断系统
#content=$(cat /etc/wireguard/xxx.conf) && echo "${content}" | qrencode -o - -t UTF8
...
add_user(){
    ...
    sed -i 's%^Address.*$%'"Address = 10.66.0.$newnum\/24"'%' $newname.conf

cat >> /etc/wireguard/wg0.conf <<-EOF
[Peer]
PublicKey = $(cat tempubkey)
AllowedIPs = 10.66.0.$newnum/32
EOF
    wg set wg0 peer $(cat tempubkey) allowed-ips 10.66.0.$newnum/32
